...
ht=text(0,0,renderstr(1),'fontsize', rsz(1),'color',fg,...
...
hf=figure('Visible','off'); clf; im(P,[],0); truesize; hold on;
ht=text(0,0,renderstr(2),'fontsize',rsz(1),'color',fg,...
  'fontweight',weights{rweight},....
  'fontangle',fangles{rangle},...
  'fontname',fontname,...
  'horizontalalignment','center','units','pixels');
...
truesize; 
hold on;
ht=text(0,0,renderstr,'fontsize',rsz(1),'color',fg,...
  'fontweight',weights{rweight},....
  'fontangle',fangles{rangle},...
  'fontname',fontname,...
  'horizontalalignment','center','units','pixels');
...
angle = 5 - randi(10);
R = rotationMatrix(angle*pi/180); T=[0; 0]; H=[R T; 0 0 1];
P=fevalArrays(P,@(I)imtransform2(I,H,'method','linear','bbox','crop'));
P=double(P)+randn(size(P))*25*rand; % noise
b=r(.5,2); P=uint8(gaussSmooth(P,[b b 0],'same')); % smoothing
